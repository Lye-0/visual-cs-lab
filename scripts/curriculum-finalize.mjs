// Apply reviewed text migrations and generate the authoritative module manifest.
// This script never commits, pushes, merges or deploys. The workflow checks the
// branch SHA before committing its reviewed changes.
import {readFile,writeFile,readdir,access} from 'node:fs/promises';
import {spawnSync} from 'node:child_process';
import {pathToFileURL,fileURLToPath} from 'node:url';
import assert from 'node:assert/strict';
const root=new URL('../',import.meta.url),at=path=>new URL(path,root);
const changes=[];
...
for(const name of (await readdir(at('scripts/'))).filter(n=>/^curriculum-patches.*\.json$/.test(n)).sort()){
 ...
 const items=Array.isArray(plan)?plan:(plan.patches||[]);
 for(const item of items){
  ...
  // A previous reviewed migration may already have changed the same expression.
  // Do not overwrite such changes: syntax/model/independent tests remain gates.
...
const manifestPath=at('scripts/modules.mjs'),old=await import(pathToFileURL(fileURLToPath(manifestPath)).href+'?finalize');
...
// Replace only the old total assertion, never the checks that preserve old IDs.
...
// Do not downgrade a version changed independently by the repository owner.
...
for(const directory of ['src','scripts','tests'])for(const name of await readdir(at(directory+'/'))){
 ...
 assert.equal(result.status,0,result.stderr||directory+'/'+name+' syntax error');
}
