...
`experiences-view.js` はモデルが返した同じフレームの説明・図表・値を表示します。計算記録では6件ずつ追加し、既存の記録や選択を残します。結果の導出は開いたときに生成し、同じく6件ずつ追加します。図表の選択はその記録内へ適用し、最終フレームへ置き換えません。入力変更後は直前の結果を表示したまま再計算し、更新中であることを示します。前の図の操作は止め、新しい結果の準備ができてから差し替えます。
