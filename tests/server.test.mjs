...
import {shellModules,shellStyles} from '../scripts/delivery.mjs';
...
test('読み取り専用ローカルHTTPサーバー',async t=>{
 ...
 try{
  ...
  await t.test('軽量HTMLが同じ公開元の分離JSとCSSを読み込む',async()=>{
   ...
   assert.match(r.body,/script-src 'self'/);assert.equal([...r.body.matchAll(/<script defer src=/g)].length,shellModules.length);assert.equal([...r.body.matchAll(/rel="stylesheet"/g)].length,shellStyles.length);
  ...
  await t.test('私的資料・開発情報を静的配信しない',async()=>{for(const file of ['/.git/config','/.kilo/config','/private-notes/example.txt','/work/scratch.txt','/notes.pdf'])assert.equal((await get(port,file)).status,403);});
