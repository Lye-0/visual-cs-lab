// Full-scope verification. Run against committed sources; no source repairs here.
...
const out='review-output/full-audit',capture=name==='chromium',server=spawn(process.execPath,['scripts/server.mjs'],{env:{...process.env,PORT:'4217'},stdio:['ignore','ignore','pipe']});
...
async function inspect(page){return page.evaluate(()=>{
 ...
 for(const svg of root.querySelectorAll('svg')){
  ...
  for(const el of svg.querySelectorAll('text')){
   ...
   try{const b=el.getBBox(),style=getComputedStyle(el);if(b.x<vb.x-3||b.y<vb.y-3||b.x+b.width>vb.x+vb.width+3||b.y+b.height>vb.y+vb.height+3)candidates.push({type:'svg-text-outside-viewbox',text:el.textContent.slice(0,100),box:{x:b.x,y:b.y,w:b.width,h:b.height},viewBox:[vb.x,vb.y,vb.width,vb.height]});if(style.fill==='rgb(0, 0, 0)')candidates.push({type:'black-svg-text',text:el.textContent.slice(0,100)});}catch{}
  }
 }
...
try{
 ...
 for(const width of report.widths){
  ...
  for(const unit of units)for(const chapter of unit.chapters){
   ...
   try{
    ...
    const view=await inspect(page);row.observed={...view};delete row.observed.text;row.textHash=digest(normalize(view.text));
    // Warnings such as SVG label positions remain visible in the report. They
    // are never hidden simply to make the structural assertion green.
 ...
 // One line per actual definition is easy to inspect without executing code.
