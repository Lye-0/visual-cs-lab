...
try{
 ...
 for(const width of [1440,390,320]){
  ...
  await page.locator('.ex-kind-ledger [name=rhs]').fill('6,1');assert.ok(await page.locator('[data-ex-evidence]').count()>0);assert.equal(await page.locator('.ex-kind-ledger [data-ex-result]').evaluate(e=>e.inert),true);await page.locator('.ex-kind-ledger button[type=submit]').click();await page.waitForSelector('[data-ex-result][data-update-state=ready]');assert.match(await record.innerText(),/6/);checks.push(width+': editing marks retained evidence stale and execution replaces it');
