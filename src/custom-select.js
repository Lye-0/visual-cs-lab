...
function start(){
 ...
 document.addEventListener('click',e=>{const label=e.target.closest('label'),select=label?.control;if(select?.tagName!=='SELECT'||e.target.closest('button,a'))return;const button=controls.get(select);if(button&&!button.disabled){e.preventDefault();button.focus();open(select,button);}},true);
