...
function control(ctrl,params){
 ...
 if(ctrl.type==='code')return `<div class="control code-control"><label class="control-label" for="${id}">${h(ctrl.label)}</label><textarea id="${id}" data-param="${ctrl.key}" spellcheck="false" ${aria}>${h(v)}</textarea>${help}<div class="control-help">入力に合わせて自動で計算し直します。コードは教材モデルで処理します。</div></div>`;
...
A.syncControls=()=>{
 ...
 for(const ctrl of c.lab.controls){
  ...
  const number=document.getElementById('num-p-'+ctrl.key);if(number&&number!==document.activeElement&&!c.invalidInputs?.[ctrl.key])number.value=c.params[ctrl.key];
 }
...
A.runCurrent=async(options={})=>{
 ...
 if(Object.keys(c.invalidInputs||{}).length){c.pending=false;c.dirty=true;c.result=null;c.error=Object.values(c.invalidInputs).join(' ');A.markPending();A.renderDetails();A.inputStatus(c.error);return false;}
 ...
 try{
  ...
  A.inputStatus(cryptoError?'通常のブラウザで実行してください。':'入力を修正すると自動で計算し直します。');A.renderDetails();A.updatePlayer();
  return false;
 }
};
A.updatePlayer=()=>{
...
A.parameter=(key,value,{immediate=false,code=false}={})=>{
 const c=A.current;if(!c||!c.lab.controls.some(x=>x.key===key))return;if(c.invalidInputs)delete c.invalidInputs[key];
 if(c.params[key]===value){
  ...
  if(c.dirty){clearTimeout(A.parameterTimer);A.parameterTimer=setTimeout(()=>{if(A.current===c)A.runCurrent();},immediate?0:240);}
  return;
 }
 ...
 A.inputStatus('入力に合わせて更新しています…');
 A.parameterTimer=setTimeout(()=>{if(A.current===c)A.runCurrent();},code?450:immediate?0:200);
};
A.example=async kind=>{
 const c=A.current;if(!c)return;c.invalidInputs={};clearTimeout(A.parameterTimer);A.stop();
...
A.renderDetails=()=>{
 ...
 if(c.tab==='understand'){
  ...
  if(!ready)body+='<div class="info-banner">入力に対応する計算が終わると比較できます。計算前の値は比較に使いません。</div>';
  else if(b){
