...
X.registerWidget('gradient-desk',(root,a,c)=>A.mount(root,c,{
 ...
 action:(code,fields,state)=>{
  const [kind,j,i]=code.split(':');if(kind==='select')return {kind,example:Number(j)};if(kind==='weight')return {kind,unit:Number(j),feature:Number(i)};
  return kind==='edit'?{kind,value:fields.get('weight-'+state.unit+'-'+state.feature+'-'+state.iterations)}:kind==='update'?{kind,rate:fields.get('rate')}:{kind};
 },
 render:(s,{field})=>{
