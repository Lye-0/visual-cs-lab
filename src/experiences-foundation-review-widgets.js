...
X.registerWidget('hamming-roles',(root,a,c)=>S.mount(root,c,{
 ...
 render:s=>{
  ...
  const bits=(values,type)=>values?`<div class="ex-fr-bits" data-fr-word="${type}">${values.map((bit,i)=>type==='channel'?`<button type="button" class="ex-button" data-sec-action="flip:${i}" aria-label="通信路・位置${i+1}のbitを反転"><small>位置${i+1}</small><strong>${bit}</strong></button>`:`<div class="${type==='received'&&((i+1)&s.mask)?'ex-fr-source':''}" data-fr-position="${i+1}"><small>位置${i+1}</small><strong>${bit}</strong><small>${labels[i]}</small></div>`).join('')}</div>`:p('まだありません。');
...
X.registerWidget('trapezoid-select',(root,a,c)=>{
 const ui=S.mount(root,c,{
  ...
  render:(s,{field})=>{
   ...
   box('選んだ区間'+(s.selected+1)+'の一項を読む',table(['どの量か','今回の値'],[['始点側の計算端点 xᵢ',F(sel.x0)],['次の計算端点 xᵢ₊₁',F(sel.x1)],['始めの高さ f(xᵢ)',F(sel.y0)],['次の高さ f(xᵢ₊₁)',F(sel.y1)],['符号付きの幅 Δx',F(sel.dx)]])+formula('この項 = Δx × {f(xᵢ)+f(xᵢ₊₁)}/2')+formula(F(sel.dx)+' × ('+F(sel.y0)+' + '+F(sel.y1)+') / 2 = '+F(sel.contribution)))+
...
X.registerWidget('knapsack-cells',(root,a,c)=>S.mount(root,c,{
 ...
 render:(s,{field})=>{
  ...
  return box('品物はそれぞれ1個だけ使える',table(['品物','重さ','価値'],s.items.map(i=>[i.name,i.weight,i.value]))+`<form class="ex-fr-form">${field('capacity','バッグの容量',s.capacity,{min:0,max:12})}${b('容量を変えて計算する','capacity')}</form>`)+box('結果のセルから二つの参照元へ戻る',matrix+p('押されているセルが今回の答え。青緑枠は入れない場合、橙枠は入れる場合の参照元です。'))+box('選んだセルの計算',explanation)+`<details class="ex-fr-details" data-sec-view="dp-reconstruction"><summary>全品物・現在の容量で選ぶ一例を逆にたどる</summary>${table(['今回のセル','選ぶか','次に見る容量'],v.trace.map(t=>['dp['+t.row+']['+t.col+'] = '+t.value,t.item+(t.chosen?'を入れる':'を入れない'),t.nextCol]))}${p('選ぶ一例：'+(v.bag.join('、')||'なし')+'。最大価値：'+v.optimum+'。バッグを隙間なく満たすことではなく、容量以内の価値が目的です。')}</details><details class="ex-fr-details" data-sec-view="dp-items"><summary>品物の条件を変えてみる</summary>${s.items.map((i,index)=>box(i.name,field('weight-'+index,'重さ',i.weight,{min:1,max:8})+field('value-'+index,'価値',i.value,{min:0,max:12})+b(i.name+'の条件を反映','item:'+index))).join('')}</details>`;
 }
}));
})();
