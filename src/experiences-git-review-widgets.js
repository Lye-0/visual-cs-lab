...
const paired=(heads,rows)=>'<dl class="ex-git-paired">'+rows.map(row=>'<div><dt>'+h(String(row[0]))+'</dt><dd><span>'+h(heads[1])+'</span><pre>'+h(String(row[1]))+'</pre></dd><dd><span>'+h(heads[2])+'</span><pre>'+h(String(row[2]))+'</pre></dd></div>').join('')+'</dl>';
const changes=rows=>rows.length?paired(['ファイル','変更前','変更後'],rows.map(r=>[r.file,literal(r.before),literal(r.after)])):p('差分はありません。');
...
function graph(s,label,token,interactive=true){
 const r=s.repo,v=Q.view(s),ids=v.commits.map(c=>c.id),main=new Set(),active=G.ancestors(r,G.oid(r));
 for(let id=r.refs.main;id;id=r.objects[id]?.parents[0])main.add(id);
 const referenced=new Set([...Object.values(r.refs),...Object.values(r.tracking)].flatMap(id=>[...G.ancestors(r,id)]));
 const positions=Object.fromEntries(ids.map((id,i)=>[id,[52+(main.has(id)?0:active.has(id)||referenced.has(id)?1:2)*66,36+i*60]]));
 ...
 const nodes=ids.map(id=>{const [x,y]=positions[id],commit=r.objects[id],labels=[...Object.entries(r.refs).filter(([,v])=>v===id).map(([k])=>k),...Object.entries(r.tracking).filter(([,v])=>v===id).map(([k])=>k),G.oid(r)===id?'HEAD':''].filter(Boolean).join(' / ');return `<g ${interactive?`data-sec-action="select:${id}" role="button" tabindex="0" aria-label="コミット${id}を選ぶ"`:''} class="ex-git-node${active.has(id)?'':' is-old'}${s.selected===id?' is-selected':''}"><rect class="ex-git-hit" x="20" y="${y-22}" width="550" height="44" rx="7"/><circle cx="${x}" cy="${y}" r="10"/><text x="${x+24}" y="${y+5}">${h(id+'　'+labels)}</text></g>`;}).join('');
 return `<div class="ex-git-graph-wrap" data-sec-view="${token}-scroll" tabindex="0" role="region" aria-label="${h(label)}"><svg viewBox="0 0 590 ${Math.max(120,ids.length*60+25)}" class="ex-git-graph" role="group" aria-label="${h(label)}"><title>${h(label)}</title><defs><marker id="${token}" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M0 0L10 5L0 10Z"/></marker></defs>${edges}${nodes}</svg></div><p class="ex-git-scroll-help">図が入りきらない場合は、枠の中を横へスクロールできます。</p>`+p('線の矢印はコミットから親へ向かいます。縦方向は作成順の目安で、横の列はブランチの所有権ではありません。破線の点は現在のHEADの祖先でないオブジェクトです。');
}
...
const renderers={
 ...
 `<div class="ex-git-three">${box('1　作業ファイル / worktree',files(s.repo.work)+field('content','新しい内容',s.repo.work[s.file]??'',{type:'textarea',maxLength:400})+b('作業内容だけを変更','edit')+p('入力欄は下書きです。ボタンを押すと上の作業内容へ反映します。'))}${box('2　次に保存する内容 / index',files(s.repo.index)+p('対象：'+s.file)+b('このファイルをaddする','add')+p('addは、その瞬間の内容をindexへ写します。以後の編集が自動で追従するわけではありません。'))}${box('3　保存済み / HEAD',p('HEAD → '+(s.repo.head||'detached')+' → '+G.oid(s.repo))+files(s.repo.objects[G.oid(s.repo)].tree)+field('message','コミットの説明','変更を保存',{type:'text'})+b('indexをcommitする','commit')+p('indexがHEADと同じなら新しい変更はありません。'))}</div>`+
 `<div class="ex-git-two">${box('未ステージ差分 / index → worktree',changes(v.unstaged))}${box('ステージ済み差分 / HEAD → index',changes(v.staged))}</div>`+graph(s,'保存されたコミット',token)+commitDetail(s);},
 ...
 compareRepos(s)+graph(s,'reset後も残るコミット',token)+commitDetail(s)+p('参照をC0へ戻した場合でも、C1がこの教材のオブジェクト一覧に残ることを確認してください。実Gitでもreset直後にコミットを物理削除するわけではありませんが、保存していない作業内容までreflogから回復できるわけではありません。GC・未追跡ファイル・path指定のresetはこの例の範囲外です。');},
 ...
 remote(s,field,token){const r=s.repo,v=Q.view(s);return p('別の開発者による変更を作り、fetchの前後を比べます。「相手側の実際のmain」パネルは実験者向けで、ローカルがまだ取得していない情報も含みます。')+
...
for(const lesson of ['stage','reset','merge','rebase','remote','undo'])X.registerWidget('git-'+lesson+'-desk',(root,a,c)=>{
 ...
 const ui=S.mount(root,c,{start:()=>Q.start(lesson),reduce:Q.reduce,instruction:'教材内の仮想Gitです。実ファイルやGitHubは変更しません。',
 action:(code,f,s)=>{const [kind,value]=code.split(':');
  ...
  return {kind};
 },render:(s,{field})=>renderers[lesson](s,field,token)+`<details class="ex-git-details" data-sec-view="git-scope"><summary>この仮想Gitで扱う範囲</summary>${p(scopes)}</details>`});
