...
X.registerWidget('causal-clocks',(root,a,current)=>{
 ...
 ui.scope.on(root,'submit',e=>{if(!e.target.matches('[data-clock-compare]'))return;e.preventDefault();first=Number(e.target.elements.first.value);second=Number(e.target.elements.second.value);ui.refresh('compare');ui.message('選んだ二つの因果の道と時計を比較しました。');});
});
